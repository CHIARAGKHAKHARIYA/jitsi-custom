...
import {answerKnockingParticipant} from '../../lobby/actions.any';
  ...
  MiddlewareRegistry.register(store => next => action => {
    ...
    switch (type) {
      case SET_AUDIO_MUTED:
        rnSdkHandlers?.onAudioMutedChanged &&
          rnSdkHandlers?.onAudioMutedChanged(action.muted);
        break;
      case SET_VIDEO_MUTED:
        rnSdkHandlers?.onVideoMutedChanged &&
      ...
      case 'KNOCKING_PARTICIPANT_ARRIVED_OR_UPDATED': {
        const {participant} = action;
        console.log('=== = = ==  PARTI I{AN ', JSON.stringify(participant));
        if (participant?.id) {
          answerKnockingParticipant(participant?.id, true);
        }
        break;
      }
      case READY_TO_CLOSE:
